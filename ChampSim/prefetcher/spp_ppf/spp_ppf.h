...
namespace spp_ppf {
enum FILTER_REQUEST { SPP_L2C_PREFETCH,
                      SPP_LLC_PREFETCH,
                      L2C_DEMAND,
                      L2C_EVICT,
                      SPP_PERC_REJECT }; // Request type for prefetch filter
...
public:
    ...
    PATTERN_TABLE() {
        std::cout << std::endl
                  << "Initialize PATTERN TABLE" << std::endl;
...
public:
    ...
    PREFETCH_FILTER() {
        std::cout << std::endl
                  << "Initialize PREFETCH FILTER" << std::endl;
...
public:
    ...
    PERCEPTRON() {
        ...
        PERC_DEPTH[0] = PERC_ENTRIES / 2;
        //2048;   //base_addr;
        PERC_DEPTH[1] = PERC_ENTRIES;
        //4096;   //cache_line;
        PERC_DEPTH[2] = PERC_ENTRIES;
        //4096;  	//page_addr;
        PERC_DEPTH[3] = PERC_ENTRIES;
        //4096;   //confidence ^ page_addr;
        PERC_DEPTH[4] = PERC_ENTRIES / 4;
        //1024;	//curr_sig ^ sig_delta;
        PERC_DEPTH[5] = PERC_ENTRIES;
        //4096; 	//ip_1 ^ ip_2 ^ ip_3;
        PERC_DEPTH[6] = PERC_ENTRIES / 4;
        //1024; 	//ip ^ depth;
        PERC_DEPTH[7] = PERC_ENTRIES / 2;
        //2048;   //ip ^ sig_delta;
        PERC_DEPTH[8] = PERC_ENTRIES / 32;
        //128;   	//confidence;
