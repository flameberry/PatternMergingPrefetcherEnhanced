...
# (Aditya): This is to generate compile_commands.json for editor intellisense to work properly
make_prefix = ""
...
def modify_config_1core(branch, prefetcher, name):
    ...
    load_dict["L1D"]["prefetcher"] = (
        prefetcher[1:] if (prefetcher[0] == "v") else prefetcher
    )
...
def make_1core(prefetcher):
    ...
    os.system(f"make clean && {make_prefix}make -j16")
...
def modify_config_1core_multi_level(branch, prefetchers, name):
    ...
    load_dict["L1D"]["prefetcher"] = (
        prefetchers[0][1:] if prefetchers[0][0] == "v" else prefetchers[0]
    )
...
def modify_config_1core_system_sensitivity(
    branch, prefetcher, bw, llc_size, l2c_size, name
):
    if (
        bw not in [800, 1600, 3200, 6400, 12800]
        or llc_size not in [0.5, 1, 2, 4, 8]
        or l2c_size not in [0.125, 0.25, 0.5, 1, 1.5]
    ):
...
def make_1core_system_sensitivity(prefetcher):
    ...
    for bw in [800, 1600, 6400, 12800]:
        ...
        modify_config_1core_system_sensitivity(
            "hashed_perceptron", prefetcher, bw, llc_size, l2c_size, name
        )
        os.system("./config.sh champsim_config_auto.json")
        os.system(f"make clean && {make_prefix}make -j16")
